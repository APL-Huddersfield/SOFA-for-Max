...
typedef enum _sofa_msbrir_outlets {
    BANG_OUTLET = 0,
    DUMP_OUTLET
}t_sofa_msbrir_outlets;

void *sofa_msbrir_new(t_symbol* s, long argc, t_atom* argv);
void sofa_msbrir_free(t_sofa_msbrir* x);

void sofa_msbrir_setSofa(t_sofa_msbrir* x, t_symbol* s);
void sofa_msbrir_setBuffer(t_sofa_msbrir* x, t_symbol* s);
bool sofa_msbrir_registerBuffer(t_sofa_msbrir* x, t_symbol *s);

void sofa_msbrir_getBlock(t_sofa_msbrir* x, t_symbol* s, long argc, t_atom* argv);
void sofa_msbrir_getMRE(t_sofa_msbrir* x, t_symbol* s, long argc, t_atom* argv);
void sofa_msbrir_get(t_sofa_msbrir* x, t_symbol* s, long block, t_symbol* bufferName, long channel);

void sofa_msbrir_getSize(t_sofa_msbrir* x);
void sofa_msbrir_getSizeSamps(t_sofa_msbrir* x);

bool sofa_msbrir_isSofaValid(t_sofa_msbrir* x, t_symbol* s);
t_max_err sofa_msbrir_notify(t_sofa_msbrir* x, t_symbol* s, t_symbol* msg, void* sender, void* data);
t_max_err sofa_msbrir_attrSetSofa(t_sofa_msbrir* x, t_object* attr, long argc, t_atom* argv);
t_max_err sofa_msbrir_attrGetSofa(t_sofa_msbrir* x, t_object* attr, long* argc, t_atom** argv);

void sofa_msbrir_assist(t_sofa_msbrir* x, void* b, long m, long a, char* s);

void *sofa_msbrir_class;


void ext_main(void *r) {
	t_class *c;

	c = class_new("sofa.msbrir~", (method)sofa_msbrir_new, (method)sofa_msbrir_free, (long)sizeof(t_sofa_msbrir), 0L, A_GIMME, 0);

    class_addmethod(c, (method)sofa_msbrir_getSize,        "getsize",      A_NOTHING, 0);
    class_addmethod(c, (method)sofa_msbrir_getSizeSamps,   "getsizesamps", A_NOTHING, 0);

    class_addmethod(c, (method)sofa_msbrir_setSofa,        "set",          A_SYM, 0);
    class_addmethod(c, (method)sofa_msbrir_setBuffer,      "setbuffer",    A_SYM, 0);
    class_addmethod(c, (method)sofa_msbrir_getBlock,       "getblock",     A_GIMME, 0);
    class_addmethod(c, (method)sofa_msbrir_getMRE,         "get",          A_GIMME, 0);
    class_addmethod(c, (method)sofa_msbrir_notify,         "notify",       A_CANT, 0);
	class_addmethod(c, (method)sofa_msbrir_assist,         "assist",       A_CANT, 0);

    CLASS_ATTR_SYM(c, "sofaobject", 0, t_sofa_msbrir, sofa_name);
    CLASS_ATTR_ACCESSORS(c, "sofaobject", sofa_msbrir_attrGetSofa, sofa_msbrir_attrSetSofa);
...
void sofa_msbrir_setBuffer(t_sofa_msbrir* x, t_symbol* s) {
    if(!sofa_msbrir_registerBuffer(x, s)) {
...
void sofa_msbrir_getBlock(t_sofa_msbrir* x, t_symbol* s, long argc, t_atom* argv) {
    ...
    if(!sofa_msbrir_isSofaValid(x, s)) {
        return;
    }
    ...
    sofa_msbrir_get(x, s, block, bufferName, bufferChannel);
}
...
void sofa_msbrir_get(t_sofa_msbrir* x, t_symbol* s, long block, t_symbol* bufferName, long channel) {
    ...
    /*else if(buffRef == NULL) {
        ...
        if(!x->buffSet) {
            x->buffSet = sofa_msbrir_registerBuffer(x, x->buffName);
            if(!x->buffSet) {
                object_error((t_object*)x, "%s: no destination buffer has been set", s->s_name);
                return;
            }
...
bool sofa_msbrir_isSofaValid(t_sofa_msbrir* x, t_symbol* mess) {
    if(!x->isBoundToSofa) {
    ...
    if(x->sofa_ob->sofa->convention != SOFA_MULTISPEAKER_BRIR) {
        object_error((t_object*)x, "%s: incompatible convention. sofa.msbrir~ expects a MultiSpeakerBRIR", mess->s_name);
        return false;
    }
    return true;
}
    ...
    if(msg == gensym("globalsymbol_binding")) {
        if(x->sofa_name) {
            if(object_findregistered(APL_SOFA_NAMESPACE, x->sofa_name)) {
                sofa_msbrir_setSofa(x, x->sofa_name);
            }
        }
    }
...
void *sofa_msbrir_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if((x = (t_sofa_msbrir *)object_alloc(sofa_msbrir_class))) {
        x->sofa_ob = NULL;
        x->isBoundToSofa = false;
        if(sofa_name) {
            x->sofa_name = sofa_name;
            sofa_msbrir_setSofa(x, sofa_name);
        }
        x->buffRef = NULL;
        x->buffName = buff_name;
        x->buffSet = false;
        if(buff_name) {
            x->buffSet = sofa_msbrir_registerBuffer(x, buff_name);
        }
